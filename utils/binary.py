...
import math
from sab_utils import high_order_residual_alternating_order1_rc_nomean_omega, high_order_residual_alternating_order2_rc_nomean_omega
index = 0
@torch.no_grad()
...
class Binarization(nn.Module):
    ...
    def quantize(self, w, mask, order=2, groupi=0, S=None):
        ...
        elif self.method == 'sab':
            if order == 2:
                w = high_order_residual_alternating_order2_rc_nomean_omega(w, mask, order=order, omega=omega)
            else:
                w = high_order_residual_alternating_order1_rc_nomean_omega(w, mask, order=order, omega=omega)  
